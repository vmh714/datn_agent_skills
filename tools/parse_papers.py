...
pdf_files = [
    os.path.join(pdf_dir, "sensors-17-00198.pdf"),
    os.path.join(pdf_dir, "Wearable_Sensors-Based_Human_Fall_Detection_Employing_Cuckoo_Search_Optimization_and_Hidden_Markov_Model.pdf")
]
...
def process_paper(pdf_path):
    ...
    base_name_guess = os.path.splitext(filename)[0]
    # We don't know the exact new filename until Gemini returns it, but we can check if there's any file in output_dir that was parsed from this.
    # Actually, if the file was already renamed, it won't have unclear names.
    # We can just check if a parsed markdown file already exists for the current filename.
    possible_out_path = os.path.join(output_dir, f"{base_name_guess}_parsed.md")
    if os.path.exists(possible_out_path):
        print(f"[-] Bỏ qua {filename} vì đã được parse.")
        return True
    ...
    try:
        ...
        # Chỉ lấy 15,000 ký tự đầu tiên để gửi cho Gemini lấy BibTeX và Summary nhằm tiết kiệm Token
        snippet_for_ai = full_md_text[:15000]
            
        print("  -> Gửi tới Gemini API (chỉ gửi phần đầu để tiết kiệm token)...")
        ...
        for attempt in range(len(API_KEYS)):
            try:
                ...
                # NỐI TÓM TẮT, BIBTEX VÀ TOÀN BỘ TEXT CỦA BÀI BÁO LẠI VỚI NHAU
