...
│           ├── data_collection.py# POST /sessions (raw IMU → InfluxDB imu_raw)
...
## PostgreSQL Schema (sau 4 Alembic migrations)
```
...
devices:       device_id(str PK), firmware_version, current_wearer_id[unique FK], is_active,
...
|-------------|------|--------|
| `telemetry` | device_id, wearer_id | battery_pct, walk_steps, run_steps, distance_m |
| `imu_raw` | device_id, label, session_id | ax, ay, az, gx, gy, gz |
...
| POST | `/api/v1/data-collection/sessions` | Lưu session IMU thô (Phase 1 dataset) |

## MQTT Service (mqtt_service.py) — Core Pipeline
Subscribe 3 topics với wildcard `+`:
- **`eldercare/+/status`** → `process_status()`: cập nhật Device.battery_pct + last_online, tính distance_m, ghi InfluxDB
