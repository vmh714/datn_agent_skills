...
- **v30_optimize (đóng gói kiến trúc kd2, BỎ KD):** `train_v30_optimize/train_v30_optimize.ipynb` — clone pipeline supervised thuần của v30 baseline, thay `build_model` = kiến trúc kd2 (GAP+GMP+Flatten + 2 SepConv/tầng + kênh 32/48/64/96, plain softmax, ~26.6k params), **bỏ hẳn teacher/Distiller**. Train chuẩn (Fall×3/Trans×0.55, augment Trans, val_loss+ModelCheckpoint). Dùng lại `cache_v30`. Kỳ vọng ≈ `kd_alpha_1_0` (acc ~0.956). Đây là framing đúng: "cải tiến kiến trúc CNN", không phải KD.
  ...
  - ✅ **FIRMWARE-VERIFIED, DEPLOY-READY** (INT8 trên ESP32-S3, bộ test 1000 mẫu): acc **0.9530** (≈ float, quantize gần như không suy hao), Idle 0.926, Trans **0.945**, Fall 0.987, **Fall recall 97.50%** (vượt baseline 96.0% & teacher 97.0%). arena **28.3KB** / tflite **55.8KB** — bằng/vượt teacher TCN (acc 0.952). So baseline firmware: Trans +9.2, Idle +6.7, Fall recall +1.5. → **kd2 thay baseline v30 làm model deploy chính.**
    - ⚠️ **LATENCY — dùng SỐ CANONICAL của báo cáo Chương 5, KHÔNG dùng 56.7ms cũ:** con số **11,20 ms** (và ~180× vs TCN 2014 ms) là phép đo chuẩn của báo cáo `5_Trien_khai_thuc_nghiem.tex` bảng `tab:mcu_arch_compare` @ **CPU 240 MHz, arena SRAM** (trọng số 55,8KB lọt cache 64KB). Số **56,7 ms / ~36×** ghi trước đây là phép đo CŨ ở điều kiện xấu (**arena trên PSRAM, trước tối ưu cache**) → **đã lỗi thời, KHÔNG dùng cho slide/report**. Trên bản deploy 160 MHz thì scale ~**17 ms** (11,2×240/160). Slide bảo vệ chốt: **11,2 ms / 180×, chú thích đo @240 MHz**. (Latency đủ nhỏ « chu kỳ window 500 ms, ~vài % duty cycle.)
