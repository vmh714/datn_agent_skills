...
## 1. Vị trí & cấu trúc
- Thư mục thật: **`REPORT/Do_an_tot_nghiep_Vu_Manh_Hung/`**. (Bản `REPORT/SOICT_DATN_Research_VIE_Template/` hoặc `REPORT/SOICT_DATN_Application_VIE_Template/` chỉ là **template trống** — KHÔNG sửa, chỉ tham khảo hướng dẫn từng mục.)
- File chính: **`20225198_VuManhHung.tex`** (preamble + thứ tự `\subfile` các chương). Ảnh: `Hinhve/`. Từ viết tắt: `Tu_viet_tat.tex`. Tài liệu tham khảo: `Danh_sach_tai_lieu_tham_khao.bib`.

| Chương | File |
|---|---|
| 1 — Giới thiệu (đặt vấn đề, mục tiêu, **nghiên cứu liên quan**, **đóng góp**) | `Chuong/1_Gioi_thieu.tex` và `Chuong/2_2_Nghien_cuu_lien_quan.tex` |
| 2 — Cơ sở lý thuyết | `Chuong/2_1_Nen_tang_ly_thuyet.tex` |
| 3 — Phương pháp & giải pháp đề xuất (Bài toán→Giải pháp→Kết quả, TinyML) | `Chuong/3_Phuong_phap_de_xuat.tex` |
| 4 — Phân tích & thiết kế hệ thống (Use case, Đặc tả, Kiến trúc) | `Chuong/4_1_Phan_tich_yeu_cau.tex` và `Chuong/4_2_Thiet_ke.tex` |
| 5 — Triển khai & đánh giá thực nghiệm (Cài đặt, Kết quả model, Thực nghiệm) | `Chuong/5_Trien_khai_thuc_nghiem.tex` |
...
- **Bắt lỗi cú pháp:** `pdflatex -interaction=nonstopmode -halt-on-error 20225198_VuManhHung.tex` (chạy ở thư mục `Do_an_tot_nghiep_Vu_Manh_Hung/`). `EXIT=0` = qua hết mọi `tikzpicture`/figure.
...
- **Xem trang/hình để verify (cực hữu ích cho TikZ & figure):** rasterize PDF → PNG rồi Read:
  `rungs -sDEVICE=png16m -r110 -dFirstPage=N -dLastPage=N -o ./_chk_p%d.png 20225198_VuManhHung.pdf` → Read `_chk_p1.png`. **Xóa ảnh debug sau khi xem.**
  Lệch trang: front-matter ~12 trang ⇒ *PDF page ≈ trang in + 12*. Lấy tổng trang từ log: `Output written on 20225198_VuManhHung.pdf (N pages`.

## 3. Glossary / Danh mục từ viết tắt
- Cơ chế **đang dùng**: định nghĩa `\newglossaryentry{label}{type=\acronymtype, name={ABBR}, description={Nghĩa VN (English)}}` trong `Tu_viet_tat.tex`; in bằng `\glsaddall` + `\printnoidxglossaries` (20225198_VuManhHung.tex ~dòng 259–264). Tự sắp ABC, 2 cột "Thuật ngữ | Ý nghĩa". **Thân bài KHÔNG cần `\gls{}`.**
- Longtable thủ công `Chuong/0_5_Danh_muc_viet_tat.tex` **đã bị comment** trong 20225198_VuManhHung.tex — bỏ qua, đừng nhầm là nguồn.
...
- ⚠️ Gotcha hay quên: nếu thêm `\usepackage`/style vào **preamble `20225198_VuManhHung.tex`**, **phải đẩy luôn `20225198_VuManhHung.tex`** — chỉ đẩy file chương sẽ khiến Overleaf thiếu package → lỗi hoặc không ra hình.
